...
def input_to_tensor(input_item: Any) -> Tensor:
    ...
    if isinstance(input_item, np.ndarray):
        return numpy_image_to_tensor(input_item)
    return torch.as_tensor(input_item)


def numpy_image_to_tensor(image: np.ndarray) -> Tensor:
    """Convert a NumPy image-like array to a tensor, preserving CHW arrays."""
    tensor = torch.as_tensor(image)
    if image.ndim == 2:
        tensor = tensor.unsqueeze(0)
    elif image.ndim == 3 and not looks_channel_first(image):
